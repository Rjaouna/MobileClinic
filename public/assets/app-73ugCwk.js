...
const playAdminNotificationSound = () => {
    try {
        ...
        const context = window.symclinicAdminAudioContext ?? new AudioContextClass();
        window.symclinicAdminAudioContext = context;
        const playTone = () => {
        ...
        // Certains navigateurs bloquent le son tant qu’aucune interaction n’a eu lieu.
    }
};
...
const getSettingsPanelIdFromHash = (root, hash = window.location.hash) => {
    ...
    const panel = target.matches('[data-settings-panel]')
        ? target
        : target.closest('[data-settings-panel]');

    return panel instanceof HTMLElement ? panel.id : null;
};

const activateSettingsTab = (root, panelId, updateHash = false) => {
    if (!(root instanceof HTMLElement)) {
        return;
    }
    ...
    tabs.forEach((tab) => {
        const isActive = tab.getAttribute('aria-controls') === activePanel.id;
    ...
    const activeTab = tabs.find((tab) => tab.getAttribute('aria-controls') === activePanel.id);
    const tabList = activeTab?.parentElement;

    if (window.innerWidth <= 900 && activeTab instanceof HTMLElement && tabList instanceof HTMLElement) {
        window.requestAnimationFrame(() => {
    ...
    if (updateHash) {
        window.history.replaceState(window.history.state, '', `#${activePanel.id}`);
        const navigation = root.querySelector('[data-settings-tabs]');

        if (navigation instanceof HTMLElement) {
            const offset = window.innerWidth <= 900 ? 74 : 16;
            const top = navigation.getBoundingClientRect().top + window.scrollY - offset;
...
const initSettingsTabs = (scope = document) => {
    scope.querySelectorAll('[data-settings-tabs-root]').forEach((root) => {
        if (!(root instanceof HTMLElement)) {
            return;
        }

        const panelId = getSettingsPanelIdFromHash(root)
            ?? root.querySelector('[data-settings-panel]')?.id
            ?? null;

        activateSettingsTab(root, panelId);

        if (root.dataset.settingsTabsReady === '1') {
            return;
        }

        root.dataset.settingsTabsReady = '1';
        const tabs = [...root.querySelectorAll('[data-settings-tab]')];
        ...
        tabs.forEach((tab, index) => {
            tab.addEventListener('click', (event) => {
                ...
                activateSettingsTab(root, tab.getAttribute('aria-controls'), true);
            });
    ...
    if (!window.symclinicSettingsTabsHashReady) {
        window.symclinicSettingsTabsHashReady = true;
        window.addEventListener('hashchange', () => {
            document.querySelectorAll('[data-settings-tabs-root]').forEach((root) => {
                const panelId = getSettingsPanelIdFromHash(root);
...
const formatLoyaltyQrTime = (seconds) => {
...
const updateLoyaltyQrCodes = (scope = document) => {
    scope.querySelectorAll('[data-loyalty-qr]').forEach((root) => {
        if (!(root instanceof HTMLElement)) {
            return;
        }

        const expiresAt = Date.parse(root.dataset.expiresAt ?? '');
        const validitySeconds = Number.parseInt(root.dataset.validitySeconds ?? '0', 10);

        if (!Number.isFinite(expiresAt) || validitySeconds <= 0) {
            return;
        }

        const remainingSeconds = Math.max(0, Math.ceil((expiresAt - Date.now()) / 1000));
        const progress = Math.min(100, Math.max(0, (remainingSeconds / validitySeconds) * 100));
        const countdown = root.querySelector('[data-loyalty-qr-countdown]');
        const progressBar = root.querySelector('[data-loyalty-qr-progress]');
        const expiredMessage = root.querySelector('[data-loyalty-qr-expired]');
        const status = root.querySelector('[data-loyalty-qr-status]');
        const isExpired = remainingSeconds === 0;

        if (countdown instanceof HTMLElement) {
            countdown.textContent = formatLoyaltyQrTime(remainingSeconds);
        }
        ...
        if (expiredMessage instanceof HTMLElement) {
            expiredMessage.hidden = !isExpired;
        }

        if (status instanceof HTMLElement) {
            status.textContent = isExpired ? 'Expiré' : 'Actif';
            status.classList.toggle('status-pill--success', !isExpired);
            status.classList.toggle('status-pill--danger', isExpired);
        }

        root.classList.toggle('is-expired', isExpired);
    });
};

const initLoyaltyQrCodes = () => {
    updateLoyaltyQrCodes();

    if (window.symclinicLoyaltyQrInterval) {
        return;
    }

    window.symclinicLoyaltyQrInterval = window.setInterval(() => {
        updateLoyaltyQrCodes();
    }, 1000);
};
